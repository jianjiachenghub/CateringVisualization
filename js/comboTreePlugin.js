(function ($, window, document, undefined) {
    ...
    ComboTree.prototype.initstruct = function () {
        for (let i = 0; i < this.copysource.length; i++) {
            ...
            for (let j = 0; j < this.copysource[i].children.length; j++) {
                this.copysource[i].children[j]['class'] = 'second'
            }
        }
    };
    ...
    ComboTree.prototype.initdom = function () {
        $(this.element).append(
            ...
            '<span _id="myplaceholder" class="input-tips select-menu-input imitationSelect role_select">可视化选项</span>'+
            '</div>' +
    ...
    ComboTree.prototype.createitem = function (SampleJSONData, container,parentName) {
        for (let j = 0; j < SampleJSONData.length; j++) {

            var oliName = SampleJSONData[j].name||SampleJSONData[j].type;
            var oliId = SampleJSONData[j].item_id||SampleJSONData[j].name;
            // li容器
            ...
            if (SampleJSONData[j]['children']) {
                ...
                this.createitem(SampleJSONData[j]['children'], subul,thisName);
            }
